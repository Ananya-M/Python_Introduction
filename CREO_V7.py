...
def create_trial_for_views(e1,vw1,vw2):
    tf=e1.get()+".txt"
    #print(path,extn,tf)
    f=open(tf, "w+")
    f.write(st)
...
def create_trial_for_model_tree(e1):
    tf=e1.get()+".txt"
    #print(path,extn,tf)
    f=open(tf, "w+")
    f.write(st)
...
def VIEWS():
    ...
    e1 = tk.Entry(master)
    e1.grid(row=0, column=1)
    tk.Label(master, text="Select Views").grid(row=1)
    vw2=tk.IntVar()
    tk.Checkbutton(master,text="ORTHO",variable=vw2).grid(row=1, column=1,pady=4)
    vw1=tk.IntVar()
    tk.Checkbutton(master,text="ISO",variable=vw1).grid(row=2, column=1,pady=4)
    
    tk.Button(master, text='Close', command=master.destroy).grid(row=3, column=0,  pady=4)
    tk.Button(master, text='Create Trail for views', command=partial(create_trial_for_views,e1,vw1,vw2)).grid(row=3, column=1,  pady=4)
    tk.Button(master, text='Run', command=run_trial).grid(row=3, column=2,  pady=4)    
    #master.mainloop()
...
def MODEL_TREE():
    ...
    e1.grid(row=0, column=1)
    tk.Button(master, text='Close', command=master.destroy).grid(row=3, column=0,  pady=4)
    tk.Button(master, text='Create Trail for Model tree', command=partial(create_trial_for_model_tree,e1)).grid(row=3, column=1,  pady=4)
    tk.Button(master, text='Run', command=run_trial).grid(row=3, column=2,  pady=4) 
