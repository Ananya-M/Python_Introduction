...
def open_cusfolders(b):
    sp=b
    path=os.path.realpath(sp)
    os.startfile(path)

def listcusfolder(e1):
       cusp=e1.get()
       array4 = next(os.walk(cusp))[1]
       array5 = []
       for local_folder in array4:
          array5.append( os.path.abspath(os.path.join(cusp, local_folder)))
       root2 = tk.Toplevel(root)
       root2.title("Custom Sub Folder Shortcut")
       root2.grab_set()
       tk.Label(root2,text="Choose the folder",justify = tk.LEFT,padx = 20).pack()
       i=0
       n=len(array4)
       while (i<n ):
               b=array5[i]
               tk.Button(root2, text=array4[i],command=partial(open_cusfolders,b)).pack()
               i=i+1

def CUS_SUB():
   root2=tk.Toplevel(root)
   root2.title("Folder Location")
   root2.grab_set()
   tk.Label(root2, text="Enter Folder Location:").grid(row=0)
   e1 = tk.Entry(root2)
   e1.grid(row=0, column=1)
   tk.Button(root2, text='Run', command=partial(listcusfolder,e1)).grid(row=4, column=1,  pady=4)
   tk.Button(root2, text='Close', command=root2.destroy).grid(row=4, column=2,  pady=4)
...
button = tk.Button(root,text="CUSTOM SUBFOLDER SHORTCUT",fg="red",command=CUS_SUB).grid(row=5,column=1)
button = tk.Button(root,text="CLOSE",fg="red",command=root.destroy).grid(row=6,column=1)
